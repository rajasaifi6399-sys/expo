...
class AssetFactory(val context: Context) {
  ...
  fun create(filePath: String, relativePath: String?): Asset {
    return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
      ...
      createAssetInDefaultLocationLegacy(filePath)
    }
  }
  ...
  private fun createAssetInDefaultLocationLegacy(filePath: String): Asset {
    TODO("XD")
//    val normalizedFilePath = normalizeAssetUri(filePath)
//    val contentUri = insertToLibrary(normalizedFilePath)
//    contentResolver.writeFileContentsToAsset(File(normalizedFilePath.path!!), contentUri)
//    val id = ContentUris.parseId(contentUri)
//    return Asset(id, context)
  }
