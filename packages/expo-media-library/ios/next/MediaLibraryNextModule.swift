...
public final class MediaLibraryNextModule: Module {
  public func definition() -> ModuleDefinition {
    ...
    AsyncFunction("createAsset") { (filePath: URL, album: Album?) async throws -> Asset in
      try await checkIfPermissionGranted()
      guard let context = appContext else {
        throw Exceptions.AppContextLost()
      }
  ...
  internal func getAssetIdsFromAssetRefs(from assetRefs: Either<[Asset], [URL]>) async throws -> [String] {
    var ids: [String] = []
  ...
  func checkIfPermissionGranted() async throws {
